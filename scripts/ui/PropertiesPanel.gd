...
var sub_asset_index: int = 0:
	set(new):
		sub_asset_index = new
		if not is_node_ready() or not photo_item:
			return
		selected_page_label.text = "%d " % [new + 1]
		total_pages_label.text = "/%d" % [photo_item.asset.get_count()]
...
@onready var total_pages_label: Label = %TotalPagesLabel
